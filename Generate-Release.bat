...
if not exist "Emulator/Src/x64/Release" (
    echo ERROR: BeebEm has not been built in Release mode. Please compile with the correct target deployment and try again.
    exit
)
...
mkdir "Elite-VR-vX.X.X"

echo Adding BeebEm to Release...
ROBOCOPY "Emulator/Src/x64/Release" "Elite-VR-vX.X.X/BeebEm" /mir
echo Success!

echo Adding Elite VR Unity Project to Release...
ROBOCOPY "Unity-Project/Build" "Elite-VR-vX.X.X/Unity-Build" /mir
echo Success!
