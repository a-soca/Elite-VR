...
BeebWin* BeebWin::instanceReference;

void BeebWin::SetBeebWin() { instanceReference = this; }; // Sets the BeebWin instance pointer Alex Soca 06/04/2025
BeebWin* BeebWin::GetBeebWin() { return instanceReference; }; // Gets the BeebWin instance pointer Alex Soca 06/04/2025
...
{
	BeebWin::SetBeebWin(); // Update the instance pointer for external use Alex Soca 06/04/2024
...
void BeebWin::ApplyPrefs()
{
	...
	ApplyCustomConfiguration(); // Alex Soca 08/03/2025
}

void BeebWin::ApplyCustomConfiguration() { // Alex Soca 25/02/2025 Custom BeebEm Configuration
	SetFreezeWhenInactive(false); // Prevent freezing when BeebEm is not focussed

	// Configure hardware
	SelectTube(TubeDevice::Acorn65C02); // Add a second 6502C as Co-Processor
	UpdateTubeMenu(); // Update the hardware menu

	//DebugOpenDialog(hInst, m_hWnd); // Turn on and open debugger

	SetFocus(m_hWnd); // Set the main BeebEm window to main focus automatically
}
