...
#define _WINSOCKAPI_   /* Prevent inclusion of winsock.h in windows.h */ // Alex Soca 23/02/2025
#include <windows.h>
#include <cstdlib>
#include <stdio.h>
#include <ctype.h>
...
#include "MemoryAccessFacade.h" // Alex Soca 24/04/2025
#include "EmulatorController.h" // Alex Soca 24/04/2025
#include "EliteStreamer.h" // Alex Soca 24/04/2025
#include <iomanip> // Used for printing hex values with fixed digits Alex Soca 30/03/2025
...
constexpr int MAX_BPS = 150;             // Max num of breakpoints/watches | Changed from 50 to 150 Alex Soca 08/03/2025
...
static const DebugCmd DebugCmdTable[] = {
	...
	{ "StreamToTerminal", DebugStreamToTerminal, "[address]", "Streams the memory at the provided address to a terminal window." },
	{ "StreamShipData", DebugStreamShipData, "", "Broadcasts the ship data to the client."}, // Alex Soca, 25/02/2025
	{ "LoadElite", DebugLoadElite, "", "Loads the Elite SSD (Must be present in same directory as exe)" }, // Alex Soca, 07/03/2025
	{ "Speed",      DebugSetEmulatorSpeed, "[s] speed", "Sets the speed of the emulator" } // Alex Soca,. 06/04/2025
};
...
void CreateTerminal() { // Create a terminal to display outputs
	...
	// Set the console handler to handle close events
	SetConsoleCtrlHandler(ConsoleHandler, TRUE);
}

// Streams 16 bytes to the terminal
void StreamToTerminalWorker(int address) { // Alex Soca 31/03/2025
	const int numberOfBytes = 16; // The number of bytes to read for each line
	int* memory; // Used to store the bytes read from memory

	while (!exitFlag.load()) {
		// Get the bytes from parasite memory
		memory = MemoryAccessor::GetMemory(address, numberOfBytes);

		// Print each byte obtained
		for (int i = 0; i < numberOfBytes; i++)
			std::cout << 
			std::fixed <<        // Set the width of the variable to be fixed
			std::setfill('0') << // Fill empty digits with 0
			std::setw(2) <<      // Print 2 digits per variable
			std::hex <<          // Convert to hexadecimal
			memory[i] << " ";    // Print the byte and add a space
		...
		// Wait for 0.25s before printing the next value
		std::this_thread::sleep_for(std::chrono::milliseconds(250)); 
	}
}

static bool DebugStreamToTerminal(const char* args) {  //Samuel Maine 18/02/2025
	CreateTerminal(); // Create a terminal to display outputs
	// Parse the memory address from the args
	int address = 0;
	if (sscanf(args, "%x", &address) != 1) {
		std::cerr << "Invalid address format. Please provide a valid hexadecimal address." << std::endl;
		return false;
	}

	// Start a new thread to handle the watch updates and output
	std::thread(StreamToTerminalWorker, address).detach();

	return true;
}

// Automatically Loads Elite
static bool DebugLoadElite(const char* /* args */) { // Alex Soca 07/03/2025
	EmulatorController::LoadDisk("elite-compendium-bbc-micro.dsd");
	return true;
}

// Streams the first 8 ship block coordinates to the client
static bool DebugStreamShipData(const char* /* args */) { // Alex Soca 25/02/2025
	// Start a new thread to handle the watch updates and output
...
static bool DebugSetEmulatorSpeed(const char* args) {
	int speed = 0;
...
void DebugOpenDialog(HINSTANCE hinst, HWND /* hwndMain */)
{
	...
	//SetDlgItemChecked(hwndDebug, IDC_DEBUGHOST, true); // Removed Alex Soca 25/02/2025

	SetDlgItemChecked(hwndDebug, IDC_DEBUGPARASITE, true); // Changed from IDC_DEBUGHOST to IDC_DEBUGPARASITE Alex Soca 25/02/2025
	SetDlgItemChecked(hwndDebug, IDC_DEBUGHOST, false); // Changed to false Alex Soca 25/02/2025

	DebugParasite = true; // Alex Soca 25/02/2025
	DebugHost = false; // Alex Soca 25/02/2025
}
